...
local akyrey_runtime = get_runtime_dir()
local akyrey_config = get_config_dir()
...
local function mod_path(path)
  ...
  path = path:gsub(akyrey_runtime .. "/", "<AKYREYVIM_RUNTIME>/")
  path = path:gsub(akyrey_config .. "/", "<AKYREYVIM_CONFIG>/")
  return path
end
