...
local logfile = string.format("%s/%s.log", vim.fn.stdpath "cache", "akyrey")
...
function Log:init()
  ...
  local notify_handler = require "akyrey.core.notify"

  ---Check if notify is available
  ---@return boolean
  local is_notify_available = function()
    local in_headless = #vim.api.nvim_list_uis() == 0
    --We can't rely on akyrey.builtin.notify.active since this can be used before the config loader
  ...
  local log_level = Log.levels[(akyrey.log.level):upper() or "WARN"]
  local akyrey_log = {
    akyrey = {
      sinks = {
  ...
  structlog.configure(akyrey_log)

  local logger = structlog.get_logger "akyrey"

  if akyrey.log.override_notify then
